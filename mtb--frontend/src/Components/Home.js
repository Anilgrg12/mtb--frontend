...
export default class Home extends Component {
  ...
  filterTrail = (type) => {
    ...
    if (type === "all") {
      ...
      sorted = [...this.state.trails].sort((a, b) =>
        a.name > b.name ? 1 : -1
      );
  ...
  render() {
    ...
    let FilteredTrail = this.state.trails.filter((trail) =>
      trail.name.toLowerCase().includes(this.state.searchTerm.toLowerCase())
    );
